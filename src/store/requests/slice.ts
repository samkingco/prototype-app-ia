...
import { actions as notificationsActions } from "../notifications/slice";
...
export function sendAvailableItems() {
  return function (dispatch: Dispatch, getState: GetState) {
    ...
    const allNotifications = notificationsSelectors.list(getState());
    const availableItems: Item[] = [];
    const doneNotificationIds: string[] = [];
    ...
    for (const item of availableItems) {
      const requestId = item.originalRequest.id;
      const notification = allNotifications.find(
        (i) => i.requestId && i.requestId === requestId
      );
      if (notification && !notification.hasBeenDone) {
        doneNotificationIds.push(notification.id);
      }
    }

    for (const notificationId of doneNotificationIds) {
      dispatch(notificationsActions.markAsDone({ notificationId }));
      dispatch(
        notificationsActions.setChosenAction({
          notificationId,
          chosenAction: "send-available",
        })
      );
    }
  };
