...
export interface FullScheduleItem extends LiveDoseScheduleItem {
    ...
    if (medication) {
      if (medication.liveDoseSchedule) {
        for (const scheduleItem of medication.liveDoseSchedule) {
          fullSchedule.push({ ...scheduleItem, medication });
        }
      }
    }
  }
...
function todaysFullScheduleByTime(state: AppState) {
  const schedule = todaysFullSchedule(state);
  const byTime = new Map<string, FullScheduleItem[]>();

  for (const scheduleItem of schedule) {
    const forTimeOfDay = byTime.get(scheduleItem.timeOfDay) || [];
    byTime.set(scheduleItem.timeOfDay, [...forTimeOfDay, scheduleItem]);
  }

  return Array.from(byTime);
}

function totalDosesRemaining(state: AppState): number {
  const allDoses = todaysFullSchedule(state);
  return allDoses.reduce(
    (total, dose) => (!dose.hasTaken ? total + 1 : total),
    0
  );
}
